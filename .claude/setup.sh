...
# compileSdk と別バージョンの platform を入れても AGP が SDK 未検出で落ちるので build.gradle.kts を正とする
compile_sdk="$(sed -n 's/.*compileSdkVersion("android-\([0-9]*\)\.0").*/\1/p' app/build.gradle.kts | head -n 1)"
if [ -z "${compile_sdk}" ]; then
  echo "app/build.gradle.kts から compileSdkVersion を読めなかった" >&2
  exit 1
fi
